      ...
      <div className="overflow-x-auto">
        <div className="max-h-96 overflow-y-auto"> {/* Fixed: Scroll box with fixed height for vertical sticky */}
          <table className="min-w-full bg-white border border-gray-300">
            <thead className="sticky top-0 bg-gray-50 z-10"> {/* Fixed: Sticky thead */}
              <tr><th className="px-2 py-2 text-left border sticky left-0 bg-gray-50 z-20"></th>{sortedAssets.map((asset)=>(<th key={asset.name} className="px-2 py-2 text-left border">{asset.name}</th>))}</tr>
            </thead>
            <tbody>
              {sortedAssets.map((assetA)=>(
                <tr key={assetA.name}>
